...
f = sns.barplot(
    data=df.loc[df['sex']== 'female',:], x="genotype", y="expression",
    hue="sex"
)
sns.stripplot(
    data=df.loc[df['sex']== 'female',:], x="genotype", y="expression", 
...
plt.gcf().savefig(f'./figures/{tag}_female.svg')
...
mgt = multiGroupTest.multiGroupTest(df["expression"],df['id'],'hypo:IndependentT','all',
                                    combination_set=[('+/+_male','+/-_male'),('+/+_male','-/-_male'),('+/-_male','-/-_male'),
                                                     ('+/+_female','+/-_female'),('+/+_female','-/-_female'),('+/-_female','-/-_female')])
