...
file = f'./Data/rei_kinematic_data.csv'
df = pd.read_csv(file)
df = df.rename(columns={'torque':'propulsion efficiency, m/deg'})
...
for expType in [('Ta','motivated swimming'),('Unt','free swimming')]:
    for parameter in ['thrust_mean_m/s', 'slip_mean_m/s', 'yaw_mean_m/s', 'thrust_median_m/s', 'slip_median_m/s', 'yaw_median_m/s', 
                      ...
                      'sec_to_first_stop', 'propulsion efficiency, m/deg']:
        f= plt.figure()
        sns.boxplot(x="genotype", y=parameter, order=['rei-INT', 'rei-HT', 'rei-HM'],
                hue="sex",hue_order=['M','F'],data=df.loc[df['expType']==expType[1],:]).set_title(expType[1])
        plt.savefig('./figures/'+f'rei_kinematics_{expType[1]}--{parameter}.svg'.replace(' ','_').replace('/','_per_'))
#plt.show()
...
for expType in [('Ta','motivated swimming'),('Unt','free swimming')]:
    df_stat = df.loc[df['expType']==expType[1],:]
    for parameter in ['thrust_mean_m/s', 'slip_mean_m/s', 'yaw_mean_m/s', 'thrust_median_m/s', 'slip_median_m/s', 'yaw_median_m/s', 
                      'cruising_thrust_mean_m/s', 'cruising_slip_mean_m/s', 'cruising_yaw_mean_m/s', 'cruising_thrust_median_m/s', 
                      'cruising_slip_median_m/s', 'cruising_yaw_median_m/s', 'activity_duration_s', 'activity_fraction', 
                      'sec_to_first_stop', 'propulsion efficiency, m/deg']:
        ...
        save_file = './stats/'+f'rei_kinematics_{expType[1]}--{parameter}_FisherMean_FDR_BH.csv'.replace(' ','_').replace('/','_per_').replace(',','')
        mgt = multiGroupTest.multiGroupTest(df_stat[parameter].to_list(),df_stat['id'].to_list(),'Fisher:meanDiff',20000,
