...
file = f'./Data/rei_curvature_c-start_data.csv'
df = pd.read_csv(file)
...
for parameter in ['median_curv_amp', 'mean_curv_amp', 'max_curv_amp']:
    ...
    plt.savefig('./figures/'+f'rei_curvature_cstart--{parameter}.svg'.replace(' ','_').replace('/','_per_'))
...
for parameter in ['median_curv_amp', 'mean_curv_amp', 'max_curv_amp']:
    ...
    save_file = './stats/'+f'rei_curvature_cstart--{parameter}_FisherMean_FDR_BH.csv'.replace(' ','_').replace('/','_per_').replace(',','')
