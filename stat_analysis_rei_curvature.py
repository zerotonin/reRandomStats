...
file = f'./Data/rei_curvature_data.csv'
df = pd.read_csv(file)
...
for expType in [('Ta','motivated swimming'),('Unt','free swimming')]:
    for parameter in ['median_curv_amp', 'mean_curv_amp', 'max_curv_amp']:
        ...
        plt.savefig('./figures/'+f'rei_curvature_{expType[1]}--{parameter}.svg'.replace(' ','_').replace('/','_per_'))
...
for expType in [('Ta','motivated swimming'),('Unt','free swimming')]:
    ...
    for parameter in ['median_curv_amp', 'mean_curv_amp', 'max_curv_amp']:
        ...
        save_file = './stats/'+f'rei_curvature_{expType[1]}--{parameter}_FisherMean_FDR_BH.csv'.replace(' ','_').replace('/','_per_').replace(',','')
